  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `你是一位專業的形象顧問和能量分析師，擅長從視覺印象中感受一個人散發的氣質與能量。
這是一個趣味娛樂性質的服務，幫助人們從藝術角度欣賞自己的獨特魅力。
回應必須使用繁體中文，並以 JSON 格式輸出。
分析要正面積極，著重於優點和潛力的描述，語氣溫暖親切。`
        },
        {
          role: "user",
          content: [
            {
              type: "text",
              text: `請從藝術欣賞的角度，描述這張照片中人物給你的整體印象和感受到的能量。這是娛樂性質的分析，請用詩意且正面的方式描述。以 JSON 格式回應：
{
  "features": ["印象特質1 - 例如：眼神充滿智慧光芒", "印象特質2", "印象特質3", "印象特質4", "印象特質5"],
  "interpretation": "200字左右的整體印象描述，包含你感受到的氣質、能量和獨特魅力，以正面積極的角度詮釋"
}`
            },
            {
              type: "image_url",
